...
class Pedido(BaseModel):
    ...
    id_marketplace: Mapped[int] = mapped_column(
        ForeignKey('marketplaces.id_marketplace'), nullable=False
    )

    id_pedido_marketplace: Mapped[int] = mapped_column(nullable=True)

    data_venda: Mapped[date] = mapped_column(nullable=False)

    valor_pedido: Mapped[Decimal] = mapped_column(nullable=False)
    ...
    marketplace_pedido: Mapped['Marketplace'] = relationship(
        foreign_keys=[id_marketplace], init=False
    )
